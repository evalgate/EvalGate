...
describe('/api/quality', () => {
  ...
  describe('GET', () => {
    ...
    it('should return 400 when action is invalid', async () => {
      const req = new NextRequest('http://localhost:3000/api/quality?evaluationId=1&action=invalid');
  ...
  describe('POST (recompute)', () => {
    it('should recompute and return score when run exists', async () => {
      const { recomputeAndStoreQualityScore } = await import('@/lib/services/aggregate-metrics.service');
