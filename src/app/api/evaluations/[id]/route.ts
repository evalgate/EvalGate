...
import { db } from "@/db";
import { evaluationRuns, evaluations, testCases, testResults, user } from "@/db/schema";
...
export const DELETE = secureRoute(async (_req: NextRequest, ctx: AuthContext, params) => {
  try {
    ...
    await db
      .delete(evaluations)
      .where(and(eq(evaluations.id, evalId), eq(evaluations.organizationId, ctx.organizationId)));
