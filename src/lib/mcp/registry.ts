...
function assertAuthContext(ctx: { authType: string; organizationId?: number }): asserts ctx is AuthContext {
...
export const MCP_TOOLS: McpToolDefinition[] = [
  {
    ...
    description: 'Get the latest quality score for an evaluation. Returns score, baseline, regression delta.',
    version: TOOL_VERSION,
    inputSchema: {
      type: 'object',
      properties: {
        evaluationId: { type: 'number', description: 'ID of the evaluation' },
        baseline: { type: 'string', description: 'Baseline mode', enum: ['published', 'previous', 'production'] },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        durationMs: { type: 'number', description: 'Duration in ms' },
        metadata: { type: 'object', description: 'Optional metadata' },
        evaluationRunId: { type: 'number', description: 'Optional evaluation run ID' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        environment: { type: 'string', description: 'Environment', enum: ['dev', 'staging', 'prod'] },
      },
      required: ['evaluationId'],
    },
...
export async function executeMcpTool<K extends ToolName>(
  ...
  switch (toolName) {
    case 'eval.quality.latest': {
      ...
      if (result === null) throw new McpToolError('NOT_FOUND', 'Evaluation not found', 404);
      return result;
    }
    ...
    case 'trace.span.create': {
      ...
      if (typeof result === 'object' && 'ok' in result && result.ok === false) {
        throw new McpToolError(
          'FORBIDDEN',
          result.reason === 'run_not_in_org' ? 'Run not in organization' : 'Trace not in organization',
          403,
        );
      }
      return result;
    ...
    default:
      throw new McpToolError('VALIDATION_ERROR', `Unknown tool: ${toolName}`, 400);
  }
}
