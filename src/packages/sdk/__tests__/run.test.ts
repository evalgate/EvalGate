...
describe("Run Command", () => {
  ...
  it("should filter by spec IDs", async () => {
    ...
    expect(result.results.map((r) => r.specId)).toEqual(expect.arrayContaining(["spec1", "spec3"]));
  ...
  it("should handle missing manifest gracefully", async () => {
    ...
    await expect(runEvaluations({}, testDir)).rejects.toThrow("No evaluation manifest found");
  });
