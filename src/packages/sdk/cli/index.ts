...
program.name("evalai").description("AI Evaluation Platform CLI").version("1.0.0");
...
program
  ...
  .action(async (options) => {
    ...
    await fs.writeFile(path.join(dir, "evalai.config.json"), JSON.stringify(config, null, 2));
...
`;
    ...
    await fs.writeFile(path.join(dir, "evaluations", "example.ts"), exampleEval);
...
program
  ...
  .action(async (options) => {
    ...
    const _client = AIEvalClient.init(config);
