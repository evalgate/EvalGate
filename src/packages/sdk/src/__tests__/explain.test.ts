...
describe("evalai explain", () => {
  ...
  it("explains a BuiltinReport (from evalai gate)", async () => {
    const report = {
      ...
      deltas: [
        { metric: "tests_passing", baseline: true, current: false, delta: "-1", status: "fail" },
        { metric: "test_count", baseline: 10, current: 8, delta: "-2", status: "fail" },
      ],
  ...
  it("reads from .evalai/last-report.json", async () => {
    ...
    fs.writeFileSync(path.join(dotDir, "last-report.json"), JSON.stringify(report, null, 2));
