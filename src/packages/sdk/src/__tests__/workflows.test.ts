...
import { createWorkflowTracer, traceWorkflowStep, WorkflowTracer } from "../workflows";
...
describe("WorkflowTracer", () => {
  ...
  describe("agent spans", () => {
    ...
    it("should throw if no workflow is active", async () => {
      await expect(tracer.startAgentSpan("Agent")).rejects.toThrow("No active workflow");
    });
  });
