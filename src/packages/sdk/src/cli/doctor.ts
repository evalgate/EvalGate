...
export async function checkEvalAccess(
  ...
  const result = await fetchQualityLatest(baseUrl, apiKey, evaluationId, baseline);
...
export function checkCiWiring(cwd: string): CheckResult & { ciInfo: DiagnosticBundle["ci"] } {
...
function printHuman(checks: CheckResult[], overall: DiagnosticBundle["overall"]): void {
  console.log("\n  evalai doctor\n");
