...
import { CHECK_REPORT_SCHEMA_VERSION, type CheckReport, type FailedCase } from "./formatters/types";
...
function classifyRootCauses(report: CheckReport): RootCauseClass[] {
  ...
  if (failedCases.length > 0) {
    ...
    const hasToolIssue = outputs.some(
      (o) => o.includes("tool_call") || o.includes("function_call") || o.includes("tool_use"),
    );
    if (hasToolIssue) {
...
function buildExplainOutput(
  ...
  if (isBuiltinReport) {
    return buildFromBuiltinReport(report as Record<string, unknown>, reportPath);
  }

  return buildFromCheckReport(report as CheckReport, reportPath);
}

function buildFromCheckReport(report: CheckReport, reportPath: string): ExplainOutput {
  const failedCases = report.failedCases ?? [];

  // Top failures (up to 3)
  const topFailures = failedCases.slice(0, 3).map((fc: FailedCase, i: number) => ({
    rank: i + 1,
    name: fc.name,
...
function buildFromBuiltinReport(
  ...
  if (failures.some((f) => f.includes("failing"))) rootCauses.push("prompt_drift");
  if (failures.some((f) => f.includes("count dropped"))) rootCauses.push("coverage_drop");
...
function printHuman(output: ExplainOutput): void {
  const verdictIcon =
    output.verdict === "pass" ? "\u2705" : output.verdict === "warn" ? "\u26A0\uFE0F" : "\u274C";
  ...
  if (output.suggestedFixes.length > 0) {
    ...
    for (const fix of output.suggestedFixes) {
      const pIcon =
        fix.priority === "high" ? "\u203C\uFE0F" : fix.priority === "medium" ? "\u2757" : "\u2022";
...
export async function runExplain(argv: string[]): Promise<number> {
  ...
  const reportSchema =
    typeof reportData.schemaVersion === "number" ? reportData.schemaVersion : undefined;
  ...
  const output = buildExplainOutput(reportData as CheckReport, path.relative(cwd, reportPath));
