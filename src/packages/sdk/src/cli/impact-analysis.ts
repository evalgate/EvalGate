	...
	return new Promise((resolve, reject) => {
		...
		git.on("close", (code) => {
			if (code !== 0) {
				...
				if (
					lowerError.includes("not a git repository") ||
					lowerError.includes("fatal: not a git")
				) {
					reject(
						new Error(
							"Not a git repository. Run 'git init' or run evalgate from inside a git repo.",
						),
					);
				} else if (
					...
					reject(
						new Error(
							`Base branch '${baseBranch}' not found. Fetch it first: git fetch origin ${baseBranch}`,
						),
					);
				} else {
					reject(
						new Error(
							`Git diff failed (exit ${code}). Ensure git is installed and '${baseBranch}' exists.`,
						),
					);
				}
				return;
...
export function printHumanResults(result: ImpactAnalysisResult): void {
	...
	if (result.impactedSpecIds.length > 0) {
		...
		console.log("\n💡 Suggested command:");
		console.log(
			`   evalgate run --spec-ids ${result.impactedSpecIds.join(",")}`,
		);
	} else {
