...
import { type EvalAIConfig, findConfigPath, loadConfig, mergeConfigWithArgs } from "./config";
...
function buildResolvedConfig(cwd: string, flags: PrintConfigFlags): PrintConfigOutput {
  ...
  const apiKeySource: Source = flags.apiKey ? "arg" : envApiKey ? "env" : "default";
  fields.push({
    key: "apiKey",
  ...
  // profile
  const profileName = (flags.profile || fileConfig?.profile) as ProfileName | undefined;
  const profileSource: Source = flags.profile ? "arg" : fileConfig?.profile ? "file" : "default";
  fields.push({
    key: "profile",
  ...
  // baseline
  const baselineSource: Source = flags.baseline ? "arg" : fileConfig?.baseline ? "file" : "default";
  fields.push({
    key: "baseline",
